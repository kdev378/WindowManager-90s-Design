...
**完了条件**:
- **M0 / M1 / M20 を実測し、SPEC §9.1.1 の判断規則を適用する**。
  これがメモリ目標に対する最初の反証機会であり、**このフェーズの最重要成果物**。
  実測値と判定結果を `docs/MEMORY.md` に記録し、削減が必要なら
  §9.1.1 の削減順序に従って以降のフェーズの範囲を確定させる
...
**グラデーションの補間規則を基準スクリーンショットとの比較で確定させる**
（SPEC §4.4。線形補間である保証は無く、ここで実測する）。
...
### Phase 4 の結果（2026-08-09、完了）

実装した: 1（taskbar.c）／2（時計）／3（スタートメニュー、ただし下記の制限あり）／
4（switcher.c）／6（tray.c）。
**実装していない: 3 の `Super` 単押し・3 の階層メニュー・4b のズームアニメーション・
5 のデスクトップウィンドウ右クリックメニュー・7 のオートハイド。**

| 項目 | 状態 | 備考 |
| --- | --- | --- |
| タスクバー（strut・タスクボタン・点滅・幅調整） | 済 | スクロール矢印は未実装（下限幅で頭打ち） |
| 時計 | 済 | `poll` タイムアウト由来。ホバーで日付は未実装 |
| スタートメニュー | 一部 | 縦バナー・キーボード操作・設定ファイル（`menu`）は済。**階層メニューは未実装（フラット 1 段）** |
| `Super` 単押しでの起動 | 未 | 既定バインドとしては `Ctrl+Esc` と `Super` の両方が引ける状態だが、sync grab + `AllowEvents(ReplayKeyboard)` によるタップ判定は入れていない。修飾キーとしての `Super` との共存は未検証 |
| Alt+Tab スイッチャ | 済 | 中央パネル、アイコン格子、下部にタイトル。Iconic を候補に含む |
| 最小化ズーム（4b） | 未 | `animate.minimize` の設定キーは読むが動作しない |
| デスクトップの右クリックメニュー（5） | 未 | `_NET_SHOWING_DESKTOP` のトグルはスタートメニューとキーバインドから可能 |
| システムトレイ | 済 | `_NET_SYSTEM_TRAY_VISUAL` に 24bit を明示。XEmbed の埋め込み・`_XEMBED_INFO`・取り外し・`kill -9` 後の生存まで `120-tray-xembed.sh` で自動検証済み。**残る未検証は「実アプリのアイコンの絵が正しく出るか」の 1 点のみ** |
| オートハイド | 未 | 設定キーは読むが動作しない |

Phase 4 で追加した検証 (`make test` は 13 本になった):

| テスト | 何を踏むか |
| --- | --- |
| `110-taskbar-tray-switcher.sh` | パネルの DOCK 宣言・strut と作業領域・最大化の下端・トレイのセレクション・Ctrl+Esc・Alt+Tab |
| `120-tray-xembed.sh` | 検証用トレイクライアント (C) を書いて XEmbed の dock / `_XEMBED_INFO` の mapped / 取り外し / 正常終了と `kill -9` 後のアイコン生存 |
| `130-swing-globally-active.sh` | 本物の Java/Swing (`tools/SwingTest.java`) を起動して ICCCM の **Globally Active** を端から端まで。打鍵がアプリに届くところまで見る |

Phase 4 で見つけて直した既存の不具合:

- **内蔵タスクバーの strut が作業領域に反映されていなかった。**
  `layout.c` は `wm.stack_bottom`（管理下クライアント）しか走査しておらず、
  override-redirect の自前パネルは載らないため素通りしていた。
- **最大化がフレームではなくクライアント領域を作業領域に合わせていた。**
  結果、フレームが装飾の分だけ外へはみ出し、**キャプションが画面外に出て
  マウスで元に戻せない**状態だった（Phase 4 でタスクバーが出るまで気づきにくかった）。
- **トレイのアイコンを save-set に入れていなかった** (§3.1.1)。
  `DestroyWindow` は所有者に関係なくサブウィンドウを道連れにするので、
  WM が死ぬとトレイのコンテナと一緒に**他プロセスのアイコンまで破棄**していた。
  フレームには最初から入れてあったのに、同じ理屈が効くトレイで抜けていた。
...
計画の妥当性は、実装で初めて検証される次の仮説に依存している。**フェーズごとに
反証されうるものとして扱い、反証されたら計画を書き換える。**

| # | 仮説 | 検証時期 | 反証されたときの行動 |
| --- | --- | --- | --- |
| 1 | `Private_Dirty` 1 MB が全機能で達成できる | **Phase 1 完了時** | SPEC §9.1.1 の削減順序を発動、または目標値を再設定 |
| 2 | Win98 のグラデーションは線形補間である | Phase 2 完了時 | 補間規則のみ差し替える（分割方式は維持） |
| 3 | 総量 12,000〜15,000 行に収まる | 各フェーズ完了時 | 実測との差を記録し、以降の見積もりを補正する。既に一度 8,400 → 12,000〜15,000 に上方修正済み |
| 4 | §7.1 の全アプリが EWMH 準拠で正常動作する | Phase 3 完了時 | 個別アプリの回避策を `docs/COMPAT.md` に記録し、必要なら仕様を追加する |
| 5 | 内蔵タスクバー/トレイのメモリ増分が 150 KB 未満 | Phase 4 完了時 | 仮説 1 と同じ判断規則を適用 |

**この計画の最大の弱点は、ここまで一度も実装からのフィードバックを受けていないこと
である。** 5 回の机上レビューで拾えたのは仕様の内部矛盾とプロトコルの誤りまでで、
「実際に測ったら/動かしたら違った」という種類の誤りは 1 件も検出できていない。
Phase 0–1 を早く終わらせて実測を得ることが、レビューを追加するより価値が高い。
