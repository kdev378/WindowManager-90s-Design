...
| 1 | コア WM | reparent / ICCCM / フォーカス / 移動 / リサイズ / スタッキング | 約 1,800 行 |
| 2 | Win98 外観 | 描画プリミティブ、フレーム、ボタン、フォント、ウィンドウメニュー | 約 1,500 行 |
| 3 | EWMH 完全対応 | 全 `_NET_*`、RandR、strut、全画面、sync request、Motif/CSD | 約 1,600 行 |
...
合計 約 8,000 行（C）。ヘッダ・テスト・スクリプトを含めて約 10,000 行を見込む。
...
- `tools/memcheck.sh` — 20 ウィンドウ生成 → `smaps_rollup` を読んで表形式で出力
...
8. スタッキング（レイヤ管理）
9. キーバインド基盤（`XGrabKey`、`MappingNotify` 対応）と `exec`
10. 終了時のクリーンアップ（全クライアントを root へ reparent し直す）

**この時点の見た目**: 装飾は単色の灰色矩形（プレースホルダ）。
**完了条件**: xterm と firefox を起動し、移動・リサイズ・フォーカス・閉じるが破綻なく動く。
...
3. `font.c` — コアフォントのオープン、`ImageText16`（UCS-2 変換）、文字幅計測、
   省略表示（`...`）。UTF-8 → UCS-2 変換を自前で持つ（約 40 行）
...
5. RandR 1.5: モニタ列挙、`RRScreenChangeNotify` によるホットプラグ追従、
   モニタ跨ぎの最大化・全画面、`_NET_WM_FULLSCREEN_MONITORS`
6. `_NET_WM_STRUT_PARTIAL` 収集 → `_NET_WORKAREA` 算出（複数パネル・複数モニタ対応）
7. `_NET_WM_SYNC_REQUEST`（XSync カウンタ、リサイズ中のフロー制御）
...
| コアフォントの環境依存（`font-misc-misc` 等が未インストール） | 文字が出ない | 起動時にフォント候補を順に試し、全滅時は内蔵 8×16 ビットマップ（ASCII のみ、約 4 KB）へフォールバック |
