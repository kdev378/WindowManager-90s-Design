...
3. **未確定リクエストは 1 つまで**：リサイズ時は §7.3 の同期カウンタが返るまで次の
   `ConfigureWindow` を送らない。移動のみの場合はカウンタを使わず 2. のレート上限で制御する。
4. **ドロップ時の確定**：ボタン解放時は上記の抑制を無視して最終ジオメトリを必ず 1 回送る。
...
**フォントの取得に失敗しても w98wm は必ず起動し、必ず文字を描ける**ことを保証する。
フォント関連の失敗は fatal error にしない。
...
| 2 | `-*-helvetica-medium-r-normal--11-*-*-*-p-*-iso10646-1` | MS Sans Serif に最も近い見た目 |
| 3 | `-*-helvetica-medium-r-normal--11-*-*-*-p-*-iso8859-1` | 同上（Latin-1 のみ） |
| 4 | `-*-lucida-medium-r-normal-sans-11-*-*-*-p-*-iso10646-1` | |
...
- 完全な多言語対応が必要な場合は `XFT=1` ビルドを推奨する旨をドキュメントに明記する。
...
| 拡張 | 用途 | 必須 |
| --- | --- | --- |
| RandR 1.2+ | モニタ列挙、ホットプラグ、`_NET_WORKAREA` 更新 | **必須**。1.5 があれば MONITOR オブジェクトを使い、1.2–1.4 では CRTC を列挙する（差分は約 30 行）。RandR 自体が無い場合はスクリーン全体を単一モニタとして扱う（マルチモニタ非対応で動作継続）。**Xinerama 経路は実装しない** |
...
**準備**：クライアントが `_NET_WM_SYNC_REQUEST_COUNTER` を持つ場合、その XSync カウンタに
対して `XSyncCreateAlarm` を 1 つ作る（`XSyncPositiveComparison`、`delta=0`）。
アラームは `XSyncAlarmNotify` イベントとして通常のイベントループに届くため、
専用のスレッドもブロッキング待ちも不要。

**リサイズ 1 コマの流れ**（クライアントごとに状態を持つ）：

```
IDLE ──(ドラッグでジオメトリ変化)──> 値を +1（奇数）して
                                     _NET_WM_SYNC_REQUEST を送信
                                     → ConfigureWindow 送信 → WAITING
WAITING ──(XSyncAlarmNotify: 値が偶数=描画完了)──> IDLE
WAITING ──(次のモーション)──> 送らずに「保留ジオメトリ」だけ更新
WAITING ──(タイムアウト 250ms 経過)──> STALLED
STALLED ──(以降)──> 同期を諦めて §3.4.1 の 16ms レート制御のみで送り続ける
                    （応答が返ったら IDLE に復帰）
```

- `WAITING` 中に溜まった保留ジオメトリは、`IDLE` に戻った瞬間に 1 回だけ送る
  （§3.4.1-3 の「未確定リクエストは 1 つまで」の実体）。
- タイムアウトは `poll(2)` のタイムアウト値に反映する。**応答しないクライアントが
  リサイズ操作全体を固まらせてはならない**（これがこの機構で最も壊れやすい点）。
- ボタン解放時は、`WAITING` であっても最終ジオメトリを必ず送って確定させる。
...
| 条件 | 内容 | 目標 | この数値が読み取れること |
| --- | --- | --- | --- |
| M0 | 起動直後・ウィンドウ 0 枚・タスクバー無効 | < 400 KB | **常駐の底**（.data/.bss・再配置・xcb 接続・起動時確保） |
| M1 | 起動直後・ウィンドウ 0 枚・タスクバー/トレイ有効 | < 550 KB | シェル部分の固定費 |
| M20 | 20 ウィンドウ・タスクバー/トレイ有効 | < 1024 KB | ウィンドウあたりの単価（M20 − M1 の 1/20 が **2 KB/窓未満**であること） |
| M20h | M20 の状態で 1 時間稼働 + 500 回の開閉/移動/リサイズ | M20 比 **+32 KB 未満** | **リークの有無**（増分が単調なら断片化ではなくリーク） |

CI ではこの 4 点すべてを記録し、いずれかが目標を超えたらビルドを失敗させる。
...
| 2 | フォント方式 | 既定は X コアフォント（常駐 0、日本語も ISO10646 フォント経由で表示可）。取得できない環境のための 9 段階フォールバックと内蔵フォントは §4.5 に規定済み。`XFT=1` でアンチエイリアス版もビルド可能にするが、Xft 有効時はメモリ目標 A を約 1.5–2 MB に緩和する必要あり（多言語表示を優先するならこちら） |
| 3 | MS Sans Serif 相当フォント | 同梱はライセンス上不可。§4.5.1 の候補列で近似する。より忠実にしたい場合、フリーの互換ビットマップ BDF を別パッケージとして案内する |
