...
| メモリ目標 | 常駐ワーキングセット 1 MB 未満（定義は §9。core ビルドのみ対象） |
| ライセンス | MIT（変更可） |
...
| 2 | フォント方式 | core ビルドは X コアフォント（常駐 0）。§4.5 のフォールバックと内蔵フォントで「必ず起動し ASCII は必ず描ける」ことを保証するが、**日本語タイトルの表示は環境依存の best-effort**（§4.5.5）。日本語を確実に出したい場合は `XFT=1` ビルドを使う想定で、こちらはメモリ目標の対象外（§1.4）。**日本語タイトルの確実な表示が必須要件かどうか**をご判断いただきたい |
