...
- 描画は `ImageText16`（UCS-2）で行う。**`ImageText16` の 1 リクエストあたりの上限は
  255 グリフ**（長さフィールドが 1 バイト）。超える場合は分割発行する。
  キャプションに 255 グリフが収まることは実際上ないが、`draw_glyphs` を `uint8_t` で
  持つ以上、上限の存在は明示しておく。
...
- **カーソル形状**はコアの `cursor` フォント（`XC_*`）から取る。依存を増やさずに済む。
  フレーム上のポインタ位置に応じて設定：
  `XC_top_left_corner` / `XC_top_side` / `XC_top_right_corner` / `XC_left_side` /
  `XC_right_side` / `XC_bottom_left_corner` / `XC_bottom_side` /
  `XC_bottom_right_corner`、キャプション上は `XC_left_ptr`、移動中は `XC_fleur`。
  カーソルは起動時に 10 個作って使い回す（`CreateGlyphCursor`。1 個あたりのサーバ側
  コストは無視できる）。
...
| 用語 | 意味 |
| --- | --- |
| `_NET_WORKAREA`（ルートプロパティ） | EWMH が定義するデスクトップごと 1 個の矩形。全モニタを合併した領域から strut を引いたもの。**外部クライアントへの通知用**であり、pager 等が読む |
...
- ラン（同一フォントで描ける連続区間）に分割して `ImageText16` を発行し、
  ベースラインを揃える。ラン分割の結果は §4.5.2.1 と同じくキャッシュする。
