 ...
 * 有界なリクエスト量で得られる。
 *
 * リクエスト数について: 段ごとに色が違うので 1 回の PolyFillRectangle には
 * まとめられない（1 リクエストは 1 つの GC = 1 色しか使えない）。
 * 実際には段ごとに ChangeGC + PolyFillRectangle の対を発行する。
 * 既定配色・幅 1920 で実測 209 段。§3.4.1 が問題にするのは
 * 送信バイト量なので、ドラッグ中の Expose 連打で効いてくるようなら
 * サーバ側 Pixmap にキャッシュして CopyArea 1 回に置き換える
 * （幅が変わった時だけ作り直す）。まず実測してから判断する。
 */
 ...
 *
 * bits のビット配置は以下に固定する。**呼び出し側は必ずこれに合わせること。**
 * ここが食い違うとグリフが静かに化けるだけで、エラーは一切出ない。
 *
 *   - 行優先 (row-major)。上の行が先。
 *   - 各行はバイト境界に切り上げてパディングする
 *     （1 行のバイト数 = (w + 7) / 8）。
 *   - 各バイトは MSB が左端 (0x80 が x=0)。
 *   - ビットが 1 の画素だけを color で描く。0 の画素には触れない
 *     （背景は呼び出し側が事前に塗る）。
 *
 * 例: w=6 のとき 1 行は 1 バイトで、下位 2 ビットは未使用。
 */
