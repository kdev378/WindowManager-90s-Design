...
/*
 * ------------------------------------------------------------------
 * xcb を使ううえでの落とし穴（このプロジェクトで実際に 2 回踏んだもの）
 * ------------------------------------------------------------------
 *
 * 1. **値リストはマスクの「ビット値の昇順」で並べる。**
 *    xcb_create_window / xcb_change_window_attributes / xcb_configure_window /
 *    xcb_create_gc / xcb_change_gc / xcb_sync_*_alarm のいずれも、
 *    値の並びはマスク式に書いた順ではなく定数のビット値順である。
 *      誤: CLIP_MASK, CLIP_ORIGIN_X, CLIP_ORIGIN_Y の順で値を置く
 *      正: CLIP_ORIGIN_X(0x20000), CLIP_ORIGIN_Y(0x40000), CLIP_MASK(0x80000)
 *    間違えるとサーバは別の属性として解釈する。BadPixmap のような
 *    分かりやすいエラーが出ることもあるが、座標がずれるだけで
 *    **何も報告されない**ことの方が多い。
 *
 * 2. **64bit の属性は 2 ワードを占める。**
 *    XSync の VALUE / DELTA など。1 ワードで数えると以降が全部ずれる。
 *
 * 3. **unchecked リクエストのエラーは後から「イベント」として届く。**
 *    どのリクエストが原因かは分からない。同期的に確かめたい箇所だけ
 *    _checked + xcb_request_check を使う (§2.2.2)。
 */
